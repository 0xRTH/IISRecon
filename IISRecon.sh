...
dirBruteForce(){
    ...
    if [[ ! -z $ffufResult ]]; then
        echo $ffufResult | tr " " "\n"| while read l; do
        echo "$(jq --arg ffufResult "$ffufResult" '.domain += {($ffufResult):{}}' result.json)" > result.json
        done
    else
        echo "$(jq --arg shortName "$1" '.domain += {($shortName):{}}' result.json)" > result.json
    fi
}
...
fileBruteForce(){
    ...
    if [[ ! -z $ffufResult ]]; then
        echo $ffufResult | tr " " "\n"| while read l; do
        echo "$(jq --arg ffufResult "$l" '.domain += {($ffufResult):[]}' result.json)" > result.json
        done
    else
        echo "$(jq --arg shortName "$1" '.domain += {($shortName):[]}' result.json)" > result.json
    fi
...
paramDiscovery(){
    ...
    if [[ ! -z $arjunResult ]]; then
        echo $arjunResult | tr " " "\n"| while read l; do
            echo "$(jq --arg arjunResult "$l" --arg file "$1" '.["domain"][($file)] += [($arjunResult)]' result.json)" > result.json
        done
    fi
    #echo "test"
}
...
cat result.json| jq '.domain' | grep "." | grep -v "~"| tr -d "\"{}\ :[],"| while read l; do
    if [[ ! -z $l ]]; then
        paramDiscovery $l
    fi
done
